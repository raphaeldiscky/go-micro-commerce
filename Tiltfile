...
# Deploy Apollo Router ConfigMaps
# Note: Router configuration is now inline in values.yaml (router.configuration)
# Note: ServiceMonitor is generated by Helm chart, not deployed separately
k8s_yaml([
    'deployments/k8s/infrastructure/apollo-router/configmap-rhai.yaml',
    'deployments/k8s/infrastructure/apollo-router/configmap-supergraph-schema.yaml',
])
...
# Register as 'graphql-gateway' for consistency
k8s_resource(
    'apollo-router',
    new_name='graphql-gateway',
    labels=['apps'],
)

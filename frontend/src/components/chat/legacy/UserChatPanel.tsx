...
export function UserChatPanel({ onRemove, user }: UserChatPanelProps) {
  ...
  const connectWebSocket = useCallback(() => {
    ...
    // Don't try to connect if we don't have ticket data yet
    if (!ticketData) {
      return
    }
    ...
    const websocketUrl = `${ticketData.node_address}/v1/ws?ticket=${ticketData.ticket}`
    console.log(`Connecting to WebSocket for ${user.name}:`, websocketUrl)
    ...
    return websocket
  }, [user.id, user.name, ticketData, refetchTicket, ws])
  ...
  useEffect(() => {
    // Only attempt to connect if we have ticket data
    if (!ticketData) {
      return
    }

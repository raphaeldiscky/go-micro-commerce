...
CREATE TABLE outbox_events (
    ...
    aggregate_type TEXT NOT NULL, -- 'cart', 'product', 'payment', etc. base on table name
    aggregate_id UUID NOT NULL,
    event_type TEXT NOT NULL, -- 'CartCreated', 'ProductUpdated', etc.
    topic TEXT NOT NULL, -- 'cart.lifecycle', 'product.lifecycle', 'user.verification', etc.
...
COMMENT ON COLUMN outbox_events.aggregate_type IS 'Type of aggregate that generated the event (cart, product, payment, etc.)';
COMMENT ON COLUMN outbox_events.aggregate_id IS 'ID of the aggregate that generated the event';
COMMENT ON COLUMN outbox_events.event_type IS 'Type of event (CartCreated, ProductUpdated, etc.)';
COMMENT ON COLUMN outbox_events.topic IS 'Kafka topic (cart.lifecycle, etc.) where the event should be published';

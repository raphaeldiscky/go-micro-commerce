...
// configuration, logger, telemetry, global providers and the asynq client.
// The returned stop func tears down telemetry and the signal context and must
// be deferred by the caller on success.
func bootstrap(parent context.Context) (*appContext, error) {
	...
	// on them (the cart service enqueues asynq tasks) are constructed.
...
// returns a cleanup func that shuts them down.

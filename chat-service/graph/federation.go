...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "Conversation":
		resolverName, err := entityResolverNameForConversation(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Conversation": %w`, err)
		}
		switch resolverName {
		...
		case "findConversationByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findConversationByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindConversationByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Conversation": %w`, err)
			}

			return entity, nil
		}
	case "Message":
		resolverName, err := entityResolverNameForMessage(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Message": %w`, err)
		}
		switch resolverName {
		...
		case "findMessageByID":
			id0, err := ec.unmarshalNUUID2githubᚗcomᚋgoogleᚋuuidᚐUUID(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findMessageByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindMessageByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Message": %w`, err)
			}
...
func entityResolverNameForConversation(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Conversation", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Conversation", ErrTypeNotFound))
			break
		}
		return "findConversationByID", nil
	}
	return "", fmt.Errorf("%w for Conversation due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}

func entityResolverNameForMessage(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Message", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Message", ErrTypeNotFound))
			break
		}
		return "findMessageByID", nil
	}
	return "", fmt.Errorf("%w for Message due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}

...
CREATE INDEX idx_connections_heartbeat ON connections (last_heartbeat) WHERE is_active = TRUE;
CREATE INDEX idx_connections_connection_id ON connections (connection_id);

-- Cleanup old inactive connections periodically
CREATE INDEX idx_connections_cleanup ON connections (last_heartbeat) WHERE is_active = FALSE;

...
// MarshalGQL implements graphql.Marshaler interface for PaymentStatus.
func (s PaymentStatus) MarshalGQL(w io.Writer) {
...
// UnmarshalGQL implements graphql.Unmarshaler interface for PaymentStatus.
func (s *PaymentStatus) UnmarshalGQL(v interface{}) error {
	str, ok := v.(string)
	if !ok {
		return errors.New("enums must be strings")
	}
	// Convert uppercase GraphQL value to lowercase Go value
	*s = PaymentStatus(strings.ToLower(str))

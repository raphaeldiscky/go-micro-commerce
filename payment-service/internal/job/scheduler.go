...
func (s *Scheduler) Start(ctx context.Context) error {
	...
	case <-time.After(s.config.PaymentTimeout.Timeout):
		s.logger.Warn("Job shutdown timeout reached, some jobs may not have stopped gracefully")
	}
...
func (s *Scheduler) GetJobStatus() map[string]any {
	s.mu.RLock()
	defer s.mu.RUnlock()

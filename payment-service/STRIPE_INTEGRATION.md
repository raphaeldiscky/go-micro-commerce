...
The payment service supports two PCI DSS compliant payment flows with **client-side tokenization using Stripe.js**:

1. **Immediate Payment Flow** - Charge the card immediately when processing payment
2. **Delayed Payment Confirmation** - Save payment method first, charge later when order is ready
...
### Delayed Payment Confirmation Architecture

This flow is used when you need to collect payment information before charging (e.g., save card first, charge later when order is ready).
...
## Delayed Payment Confirmation Flow

### Use Case

This flow is ideal when you need to:

- Collect payment details before order confirmation
- Delay charging until order is ready for fulfillment
- Support multi-step checkout processes
- Charge cards hours or days after initial collection

### Implementation

#### Step 1: Create SetupIntent (Before Order)
...
export function PaymentMethodForm({ orderId, onPaymentMethodSaved }) {
  const stripe = useStripe();
  const elements = useElements();
  const [isProcessing, setIsProcessing] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const setupResponse = await fetch('/api/v1/setup-intent', {
        method: 'POST',
        headers: {
      ...
      const { client_secret, stripe_customer_id } = await setupResponse.json();

      // Step 2: Confirm SetupIntent with Stripe.js (collect card details)
      ...
      // Step 3: Payment method successfully saved!
...
#### Step 2: Create Order with Saved Payment Method

```typescript
// After payment method is saved, create order with the IDs
async function createOrderWithPaymentMethod(
  ...
  const response = await fetch("/api/orders", {
    ...
    body: JSON.stringify({
      ...orderData,
      payment_method_id: paymentMethodId, // pm_xxx from SetupIntent
      stripe_customer_id: stripeCustomerId, // cus_xxx from backend
      payment_method: "card",
...
#### Step 3: Backend Charges Automatically

The backend will automatically charge the saved payment method when:

- Order status changes to "processing" (via Kafka event)
- The `ChargeStoredPaymentMethod` service method is triggered
...
export default function CheckoutPage() {
  ...
  const handlePaymentMethodSaved = async (pm) => {
    setPaymentMethod(pm);
    setStep(2);
  };
  ...
  const handleOrderConfirm = async (orderData) => {
    try {
      ...
      const order = await createOrderWithPaymentMethod(
        orderData,
      ...
      // Backend will charge when order status changes to PROCESSING
  ...
  return (
    <div>
      {step === 1 && (
        <div>
          <h2>Step 1: Save Payment Method</h2>
          <Elements stripe={getStripe()}>
            <PaymentMethodForm
              orderId={orderId}
      ...
      {step === 2 && (
        <div>
          ...
          <OrderConfirmationForm onSubmit={handleOrderConfirm} />
        </div>
      )}
    </div>
  );
...
#### POST /v1/orders (with payment method info)

Creates order with saved payment method IDs.
...
The payment will be automatically charged when order status changes to "processing".

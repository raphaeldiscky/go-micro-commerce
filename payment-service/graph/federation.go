...
func (ec *executionContext) resolveEntity(
	...
	case "Payment":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Payment": %w`, err)
		}
		switch resolverName {

		case "findPaymentByOrderID":
			id0, err := ec.unmarshalNUUID2githubᚗcomᚋgoogleᚋuuidᚐUUID(ctx, rep["orderId"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findPaymentByOrderID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindPaymentByOrderID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Payment": %w`, err)
			}
...
func entityResolverNameForPayment(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"orderId\" for Payment", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Payment", ErrTypeNotFound))
			break
		}
		return "findPaymentByOrderID", nil
	}
	return "", fmt.Errorf("%w for Payment due to %v", ErrTypeNotFound,

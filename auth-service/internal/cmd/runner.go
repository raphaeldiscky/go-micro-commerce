// Package cmd hosts the cobra subcommands for the auth-service binary.
// Each subcommand maps to one deployable role (serve); the "all" command
// runs every role in a single process for single-deployment setups.
package cmd
...
// configuration, logger and global providers. The returned stop func must be
// deferred by the caller on success.

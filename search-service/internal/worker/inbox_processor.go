...
func (p *InboxProcessor) processEvent(ctx context.Context, inboxEvent *entity.InboxEvent) error {
	...
	if err := p.routeEventToService(ctx, inboxEvent); err != nil {
		p.handleProcessingError(ctx, inboxEvent.ID, "failed to process search event", err)
...
func (p *InboxProcessor) routeEventToService(
	...
	p.logger.Infof("Routing event type %s from topic %s", inboxEvent.EventType, inboxEvent.Topic)

	switch inboxEvent.EventType {
	// Product events
	case "ProductCreated":
		return p.searchService.ProcessProductCreated(ctx, inboxEvent)
	case "ProductUpdated":
		return p.searchService.ProcessProductUpdated(ctx, inboxEvent)
	case "ProductDeleted":
		return p.searchService.ProcessProductDeleted(ctx, inboxEvent)

	// Order lifecycle events
	// Order events removed for now, only handling products
	case "OrderCreated", "OrderProcessing", "OrderConfirmed", "OrderPaid",
		"OrderShipped", "OrderDelivered", "OrderCanceled", "OrderFailed":
		p.logger.Infof(
			"Ignoring order event type: %s (only handling products)",
			inboxEvent.EventType,
		)
	...
	default:
		p.logger.Warnf("ignoring unknown event type for search service: %s", inboxEvent.EventType)
